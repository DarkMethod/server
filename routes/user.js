...
function handleGet(connection,req,res) {
    ...
    connection.query('SELECT * FROM user ORDER BY id DESC LIMIT ' + limit, function handleSql(err, rows) {
        if (err){ logAndRespond(err,res); return; }
...
function handleFind(connection,req,res){
     var find = function find(){
        connection.query('SELECT * FROM user WHERE id = ?', req.params.id, function handleSql(err, rows) {
            if (err){ logAndRespond(err,res); return; }
            if (rows.length === 0){ res.send(204); return; }
            res.send({
                result: 'success',
